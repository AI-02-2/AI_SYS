...
> 안내: 본 문서는 기획 시나리오와 실제 사용자 흐름을 함께 관리합니다.

## 현재 구현 반영 메모 (2026-04-17)
- OCR -> 검색 쿼리 전달 흐름이 앱에서 동작한다.
- 판례 상세에서 온디바이스 LLM 요약/퀴즈 생성 버튼 기반 흐름이 적용되어 있다.
- 홈 대시보드는 DB 기반 추천/오답 데이터를 우선 로드한다.
- 실기기 사용자는 My Page에서 API 주소를 저장해 동일 시나리오를 수행한다.
